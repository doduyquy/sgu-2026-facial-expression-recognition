...
def compute_semantic_roi_graph_losses_tf(
    ...
    # 1. Primary Classification Loss with Label Smoothing
    ...
    if train_cfg.get("enable_program_diversity", True) and hasattr(model, "program_executor"):
        ...
        w_prog = float(train_cfg.get("program_diversity_weight", 0.01))
        total_loss = total_loss + w_prog * loss_prog_div
        loss_dict["loss_prog_div"] = loss_prog_div

    # 5. Semantic Consistency Loss
    if train_cfg.get("enable_semantic_consistency", True) and "emotion_latent" in outputs:
