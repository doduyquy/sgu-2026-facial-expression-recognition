...
class CoordinateAttention(nn.Module):
    """
    Coordinate Attention with Local Enhancement.
    """
    def __init__(self, in_channels, out_channels, reduction=32):
        ...
        # Local Enhancement path to preserve fine details (mouth corners, brow furrows)
    ...
    def forward(self, x):
        ...
        return (l_feat * a_w * a_h) + identity

class MultiScaleBlock(nn.Module):
    """
    Refined Multi-Scale Block with Asymmetric Dilated Convs and Residual Scaling.
    """
    def __init__(self, in_channels, out_channels):
        ...
        # Asymmetric Dilated Branch: captures horizontal/vertical facial movements specifically
        self.branch2 = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels, kernel_size=(3, 1), padding=(2, 0), dilation=(2, 1)),
            nn.Conv2d(mid_channels, mid_channels, kernel_size=(1, 3), padding=(0, 2), dilation=(1, 2)),
            nn.GroupNorm(8, mid_channels),
            nn.GELU()
        )
        self.shortcut = nn.Conv2d(in_channels, out_channels, kernel_size=1) if in_channels != out_channels else nn.Identity()
        self.gamma = nn.Parameter(torch.zeros(1)) # Learnable scaling to stabilize dilated features
        self.att = CoordinateAttention(out_channels, out_channels)

    def forward(self, x):
        out = torch.cat([self.branch1(x), self.branch2(x)], dim=1)
        return self.att(self.gamma * out + self.shortcut(x))

class MotifBackbone(nn.Module):
    """
    Multi-Scale Dilated Backbone for FER2013.
    """
    def __init__(self, in_channels=1, feat_dim=128):
        super().__init__()
        # Stem: Preserve details by using strided conv instead of early maxpool
        ...
        # Stage 1: Local micro-features
        self.stage1 = MultiScaleBlock(64, 64)
        ...
        # Stage 2: Middle-level semantics
        self.stage2 = MultiScaleBlock(128, 128)
        ...
        self.final_refine = nn.Sequential(
            MultiScaleBlock(feat_dim, feat_dim),
            CoordinateAttention(feat_dim, feat_dim)
        )
