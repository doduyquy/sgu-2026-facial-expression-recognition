...
MODEL_REGISTRY = {
    ...
    "resnet": lambda config, **kw: ResNet50(
        ...
        use_geometry_encoding=config['model'].get('use_geometry_encoding', True),
        use_geometry_angle=config['model'].get('use_geometry_angle', False),
        use_relative_pos_bias=config['model'].get('use_relative_pos_bias', True),
        align_spread=config['model'].get('align_spread', 0.03),
