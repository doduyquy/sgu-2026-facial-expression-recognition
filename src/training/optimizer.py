...
def build_optimizer(model, config):
    ...
    for name, param in model.named_parameters():
        ...
        if (
            "backbone" in name
            and "dim_reducer" not in name
            and "final_cbam" not in name
        ):
...
def build_scheduler(optimizer, config):
    ...
    if scheduler_name == "none":
        ...
        print(
            f"--> [Scheduler] ReduceLROnPlateau (mode=max, factor={factor}, patience={patience})"
        )
...
if __name__ == "__main__":
    ...
    config_adam = {
        "training": {"optimizer": "adam", "lr": 0.001, "weight_decay": 0.0005}
    }
    opt_adam = build_optimizer(dummy_model, config_adam)
    print(f"Test 1 - Adam: Success! Type: {type(opt_adam)}")

    config_sgd = {
        "training": {"optimizer": "sgd", "lr": 0.01, "weight_decay": 0.0, "gamma": 0.95}
    }
