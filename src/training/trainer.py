...
class Trainer:
    ...
    def __init__(
        ...
        ls = (
            float(config.get("training", {}).get("label_smoothing", 0.0))
        ...
        self.motif_diversity_weight = float(
            config["training"].get("motif_diversity_weight", 0.05)
        )
        self.motif_consistency_weight = float(
            config["training"].get("motif_consistency_weight", 0.05)
        )
        self.attn_entropy_weight = float(
            config["training"].get("attn_entropy_weight", 0.01)
        )
        self.offset_reg_weight = float(
            config["training"].get("offset_reg_weight", 0.01)
        )
        self.au_contrastive_weight = float(
            config["training"].get("au_contrastive_weight", 0.03)
        )
    ...
    def _unpack_batch(self, batch):
        if isinstance(batch, (list, tuple)):
            ...
            if len(batch) == 3:
                images = batch[0]
                if (
                    getattr(batch[1], "ndim", 0) == 3
                    and getattr(batch[2], "ndim", 0) == 1
                ):
                    bboxes, labels = batch[1], batch[2]
                elif (
                    getattr(batch[1], "ndim", 0) == 1
                    and getattr(batch[2], "ndim", 0) == 3
                ):
    ...
    def train_one_epoch(self):
        ...
        for batch in self.train_loader:
            ...
            mixup_active = (
                bool(getattr(self, "_runtime_use_mixup", False)) and self.model.training
            )
            if mixup_active:
            ...
            if bboxes is not None:
                ...
                and "targets" in self.model.forward.__code__.co_varnames
            ):
            ...
            if logits is None:
                raise ValueError(
                    "Model outputs do not contain 'logits'. When returning a dict from forward(), include a 'logits' key with classification scores."
                )
            ...
            if mixup_active:
                try:
                    cls_loss = lam * F.cross_entropy(logits, labels_a) + (
                        1.0 - lam
                    ) * F.cross_entropy(logits, labels_b)
                except Exception:
                ...
                loss_dict = compute_semantic_roi_graph_losses(
                    self.model, outputs, labels, class_weights=class_weights
                )
                ...
                if _gate is not None:
                    _component_accum.setdefault("_fusion_gate_mean", []).append(
                        float(_gate.mean().detach().cpu())
                    )
                    _component_accum.setdefault("_fusion_gate_min", []).append(
                        float(_gate.min().detach().cpu())
                    )
                    _component_accum.setdefault("_fusion_gate_max", []).append(
                        float(_gate.max().detach().cpu())
                    )
                _fs = outputs.get("fusion_scale")
                if _fs is not None:
                    _component_accum.setdefault("_fusion_scale", []).append(
                        float(_fs.detach().cpu())
                    )
            else:
                if runtime_use_scn and getattr(self, "_current_epoch", 0) >= getattr(
                    self, "scn_warmup_epochs", 0
                ):
                    try:
                        cls_loss, scn_logs = self._scn_loss(logits, labels)
                        _scn_acc["scn_weight_mean"].append(
                            scn_logs.get("scn_weight_mean", 0.0)
                        )
                        _scn_acc["scn_conf_mean"].append(
                            scn_logs.get("scn_conf_mean", 0.0)
                        )
                        _scn_acc["scn_rank_loss"].append(
                            scn_logs.get("scn_rank_loss", 0.0)
                        )
                    except Exception:
                        cls_loss = self._base_criterion(logits, labels)
                else:
            ...
            if (
                "motif_consistency" in aux_losses
                and "motif_consistency" not in skip_aux
            ):
        ...
        try:
            if len(_scn_acc["scn_weight_mean"]) > 0:
                self._latest_scn_logs = {
                    "scn_weight_mean": float(
                        sum(_scn_acc["scn_weight_mean"])
                        / len(_scn_acc["scn_weight_mean"])
                    ),
                    "scn_conf_mean": float(
                        sum(_scn_acc["scn_conf_mean"]) / len(_scn_acc["scn_conf_mean"])
                    ),
                    "scn_rank_loss": float(
                        sum(_scn_acc["scn_rank_loss"]) / len(_scn_acc["scn_rank_loss"])
                    ),
                }
            else:
                self._latest_scn_logs = None
    ...
    def validate(self):
            ...
            for batch in self.val_loader:
                ...
                if bboxes is not None:
                    if (
                        ...
                        and "region_mask" in semantic_meta
                    ):
                        region_mask = semantic_meta["region_mask"].to(self.device)
                        region_confidence = semantic_meta.get("region_confidence", None)
                        if region_confidence is not None:
                ...
                if loss_mode == "semantic_roi_graph":
                    class_weights = getattr(self._base_criterion, "weight", None)
                    loss_dict = compute_semantic_roi_graph_losses(
                        eval_model, outputs, labels, class_weights=class_weights
                    )
                    cls_loss = loss_dict["loss"]
                else:
                ...
                if (
                    "motif_diversity" in aux_losses
                    and "motif_diversity" not in skip_aux
                ):
                    loss = loss + w_div * aux_losses["motif_diversity"]
                if (
                    "motif_consistency" in aux_losses
                ...
                for k, v in aux_losses.items():
                    ...
                    if k not in [
                        ...
                        w_other = self.config.get("training", {}).get(
                            f"{k}_weight", 0.1
                        )
                        loss = loss + float(w_other) * v
        ...
        macro_f1, bal_acc, per_recall, per_f1 = self._compute_metrics(
        ...
        if all_preds_motif:
            mf1_m, _, _, _ = self._compute_metrics(
                all_preds_motif, all_labels, num_classes
            )
        ...
        if all_preds_fused:
            mf1_f, _, _, _ = self._compute_metrics(
                all_preds_fused, all_labels, num_classes
            )
    ...
    def fit(self):
        ...
        self.ema_model = AveragedModel(
            self.model, multi_avg_fn=get_ema_multi_avg_fn(0.999)
        )
        ...
        for ep in range(self.epochs):
            ...
            if progress <= 0.7:
                ...
                self._runtime_motif_consistency_weight = (
                    self.motif_consistency_weight * 1.5
                )
                self._runtime_attn_entropy_weight = self.attn_entropy_weight
                self._runtime_offset_reg_weight = self.offset_reg_weight
            ...
            val_macro_f1 = getattr(self, "_latest_val_metrics", {}).get(
                "Val/MacroF1_Final", 0.0
            )
            selection_score = (
                0.5 * (val_acc.item() if hasattr(val_acc, "item") else float(val_acc))
                + 0.5 * val_macro_f1
            )
            ...
            if selection_score > best_selection_score:
                ...
                patience_counter = 0
                save_state_dict = (
                    self.ema_model.module.state_dict()
                    if hasattr(self, "ema_model")
                ...
                torch.save(
                    {
                        ...
                        "macro_f1": getattr(self, "_latest_val_metrics", {}).get(
                            "Val/MacroF1_Final", None
                        ),
                        "balanced_accuracy": getattr(
                            self, "_latest_val_metrics", {}
                        ).get("Val/BalancedAccuracy", None),
                        "val_loss": val_loss,
                    },
                    self.path_save_ckpt,
                )
