...
class Trainer:
    ...
    def _scn_loss(self, logits, labels):
        ...
        hard_idx = idx[:k]
        easy_idx = idx[k:]
        # Confidence-based Ranking (User Requested Fix: Use Confidence instead of CE Loss)
        if hard_idx.numel() > 0 and easy_idx.numel() > 0:
            conf_hard = conf[hard_idx].mean()
            conf_easy = conf[easy_idx].mean()
            # Penalize if hard confidence is too close to or higher than easy confidence
