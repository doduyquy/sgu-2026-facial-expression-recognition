...
class SymmetricCrossEntropy(nn.Module):
    def __init__(
        self, alpha=1.0, beta=1.0, num_classes=7, label_smoothing=0.0, weight=None
    ):
        super().__init__()
        self.alpha = alpha
        self.beta = beta
    ...
    def forward(self, pred, labels):
        ...
        if self.weight is not None:
            rce = (rce_per_sample * self.weight[labels]).sum() / self.weight[
                labels
            ].sum()
        else:
...
if __name__ == "__main__":
    ...
    config_motif_sce = {
        ...
        "model": {"num_classes": 7, "motifs_per_class": 8},
    }
