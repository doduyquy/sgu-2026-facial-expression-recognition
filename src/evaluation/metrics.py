...
def compute_metrics(true_labels, pred_labels):
    ...
    report = classification_report(
        true_labels, pred_labels, target_names=EMOTION_NAMES, output_dict=True
    )
...
def plot_confusion_matrix(
    true_labels, pred_labels, class_distribution, accuracy, save_path=None
):
    ...
    if class_distribution is not None:
        actual_counts = np.array(
            [class_distribution.get(i, 0) for i in labels], dtype=float
        )
    else:
